...
export function Logo({ header = false }: { header?: boolean }) { return <span className="site-logo">{header ? <span className="header-mark"><Image src={brandLogo} alt="" fill sizes="58px" priority/></span> : <span className="footer-mark"><Image src={footerLogo} alt="" fill sizes="48px"/></span>}<span>장성예찬 영농조합법인</span></span>; }
...
export function PageHead({group,title}:{group:string,title:string}) { return <><div className="breadcrumb"><Link href="/">홈</Link><span>›</span><span>{group}</span><span>›</span><b>{title}</b></div><div className="page-title"><h2>{title}</h2></div></>; }
