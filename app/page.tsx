...
export default function Home(){return <><Header/><main>
  ...
  <section className="home-notice container"><div className="section-heading"><div><h2>센터 소식</h2></div><Link href="/notices">전체보기 +</Link></div><div className="notice-cards">{notices.slice(0,3).map((n,i)=><Link href="/notices" key={i}><small>{n[0]}</small><h3>{n[1]}</h3><p>장성예찬 영농조합법인의 새로운 소식과 이용 정보를 안내합니다.</p><time>{n[3]}</time></Link>)}</div></section>
  <section className="home-products"><div className="container"><div className="section-heading light"><div><h2>가공제품 소개</h2></div><Link href="/products">전체보기 +</Link></div><div className="product-strip">{products.slice(0,4).map((p,i)=><Link href="/products" key={p}><span className="product-art product-art-image"><Image src={homeProductImages[i]} alt={`${p} 제품 이미지`} fill sizes="(max-width: 900px) 50vw, 25vw"/></span><h3>{p}</h3><p>장성의 농산물로 만든 가공제품</p></Link>)}</div></div></section>
  <section className="home-links container"><Link href="/business"><b>센터소개</b><span>주요업무와 지원사업 안내</span></Link><Link href="/education"><b>교육 프로그램</b><span>가공창업 교육 일정 안내</span></Link><Link href="/location"><b>오시는 길</b><span>장성예찬 영농조합법인 위치 안내</span></Link></section>
</main><Footer/></>}
